...
package com.hhinns.bitmap.core;
...
import com.hhinns.bitmap.BitmapGlobalConfig;
import com.hhinns.library.BitmapUtils;
import com.hhinns.library.IOUtils;
import com.hhinns.library.LogUtils;
import com.hhinns.library.LruMemoryCache;
...
public class BitmapCache
{
			...
			if (mDiskLruCache == null || mDiskLruCache.isClosed())
			{
				...
				try
				{
					mDiskLruCache = LruDiskCache.open(diskCacheDir, 1, 1,
							diskCacheSize);
					mDiskLruCache.setDiskCacheFileNameGenerator(globalConfig
							.getDiskCacheFileNameGenerator());
				} catch (Throwable e)
				{
					mDiskLruCache = null;
			...
			LruDiskCache.DiskCacheFileNameGenerator diskCacheFileNameGenerator)
	{
		...
		{
			mDiskLruCache
					.setDiskCacheFileNameGenerator(diskCacheFileNameGenerator);
		}
	}
			...
			final BitmapUtils.BitmapLoadTask<?> task)
	{
		...
		try
		{
							...
							if (snapshot == null)
							{
								...
								if (editor != null)
								{
									...
									bitmapMeta.expiryTimestamp = globalConfig
											.getDownloader().downloadToStream(
													uri, outputStream, task);
			...
			addBitmapToMemoryCache(uri, config, bitmap,
	...
	private void addBitmapToMemoryCache(String uri, BitmapDisplayConfig config,
			Bitmap bitmap, long expiryTimestamp) throws IOException
	{
		if (uri != null && bitmap != null
				&& globalConfig.isMemoryCacheEnabled() && mMemoryCache != null)
		{
			MemoryCacheKey key = new MemoryCacheKey(uri, config == null ? null
					: config.toString());
						...
						} else
						{
							bitmap = BitmapDecoder
									.decodeSampledBitmapFromDescriptor(snapshot
											.getInputStream(DISK_CACHE_INDEX)
											.getFD(),
											config.getBitmapMaxSize(), config
													.getBitmapConfig());
						}
	...
	private Bitmap rotateBitmapIfNeeded(String uri, BitmapDisplayConfig config,
			Bitmap bitmap)
	{
