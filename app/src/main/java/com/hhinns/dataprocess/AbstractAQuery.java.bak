...
//		}
	...
	public <K> T ajax_new(String url, Class<K> type, AjaxCallback<K> callback){
	...
	public <K> T ajax_new(String url, Class<K> type, Object handler, String callback){
		
		
		AjaxCallback<K> cb = new AjaxCallback<K>();
		cb.type(type).weakHandler(handler, callback);
		
		return ajax_new(url, type, cb);
